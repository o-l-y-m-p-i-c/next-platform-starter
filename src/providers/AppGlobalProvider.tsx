...
const AppGlobalProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const setAllLocalStorageData = () => {
        ...
        const storedReferralCode = localStorage.getItem(localStorageReferralCodeKey);

        const isLegalDisclaimerChecked = localStorage.getItem(localStorageLegalCheckKey);
    ...
    const setAnimationFn = useCallback(async (animationFlag?: boolean) => {
        if (typeof window !== 'undefined') {
            localStorage.setItem(localStorageAnimationKey, JSON.stringify(animationFlag));
        }
