...
import { ITwitterResponse, ITwitterTweet, TwitterSentimentThreshold } from '../../twitter.interfaces';
...
import { MindshareGraphSkeleton, TweetListSkeleton } from '@/components/TokenPageSkeleton';
...
interface TokenTwitterSentimentsProp {
    ...
    setFollowerCount: (followerCount: { minFollowers: number; maxFollowers: number }) => void;
}
...
const TokenTwitterSentiments: FC<TokenTwitterSentimentsProp> = ({
    ...
    newDatafilteredData.forEach((data_item) => {
        if (data_item.author.followers && maxFollowerCount.current < data_item.author.followers) {
    ...
    if (isLoading || dexScreenerDataIsLoading) {
        return (
            <Stack position={'relative'} gap={2}>
                <Stack position={'relative'} mx={-2} mt={-2}>
                    <MindshareGraphSkeleton variant={'rectangular'} />
                </Stack>
                <Stack>
